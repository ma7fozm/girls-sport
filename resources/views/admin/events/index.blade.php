                    ...
                    <div class="ls-editable-table table-responsive ls-table">
                        <table class="table table-bordered table-striped" id="ls-editable-table">
                            <div style="text-align:center;">
                                <a href="{{ route('event.create')}}" class="btn btn-default"
                                   style="border-radius:10px; font-size: 16px ">
                                    <i style="color: #8B8986" class="fa fa-eventspaper-o"></i> اضافة فاعليه جديده </a>
                            </div>
                            <thead>
                            <tr class="active">
                                <th class="text-center">#</th>
                                <th class="text-center"> اسم الفعاليه</th>
                            ...
                            @foreach($events as $event)
                                <tr>
                                    ...
                                    <td class="text-center">{{($event->status == 0)?'غير مفعل': 'مفعل'}}</td>
                                    <td style="width: 175px;" class="text-center">

                                        <form method="post" action="{{url('admin/events/'.$event->id)}}">
                                            {!! csrf_field() !!}
                                            {!! method_field('DELETE') !!}
                                            <button style="border-radius:5px;" class="btn btn-xs btn-danger"><i
                                        ...
                                        <a href="{{route('event.edit',['id'=>$event->id])}}" style="border-radius:5px;"
