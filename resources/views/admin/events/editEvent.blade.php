...
@section('content')
    ...
    <h1 class="text-center"><i class="fa fa-calendar"> تعديل بيانات الفاعلية </i></h1>
    <form role="form" enctype="multipart/form-data" method="post" action="{{url('admin/events/'.$event->id)}}" class="ls_form"
        ...
        <div class="form-group">
            ...
            <div class="col-md-10">
                <input type="text" name="name" class="form-control active" value="{{$event->name}}">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-md-10">
                <textarea name="agenda" class="form-control active">{{$event->agenda}}</textarea>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-md-4">
                <input id="date_timepicker_start" name="start_date" class="form-control" type="text" value="{{$event->from_datetime}}"/>
            </div>

            <label class="col-md-2 control-label"> نهاية الفاعلية :</label>
            <div class="col-md-4">
                <input id="date_timepicker_end" name="end_date" class="form-control" type="text" value="{{$event->to_datetime}}"/>
            </div>
        </div>
            ...
            <div class="col-md-10">
                <select id="select-state" name="sponsors[]" multiple class="demo-default" placeholder="اختر رعاة المباراة...">
                    @foreach($sponsors as $sponsor)
                        <option value="{{$sponsor->id}}"{{(in_array($sponsor->id,$event_sponsors_ids))?'selected':''}}>{{$sponsor->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        {{--<div class="form-group">--}}
            ...
            {{--<div class="col-md-10 ls-group-input">--}}
                {{--<select style="margin-top: 7px" id="drop" class="form-control" name="event_type">--}}
                    ...
                    {{--<option value="صحه" {{($event->event_type == 'صحه')?'selected':''}}>صحه</option>--}}
                    {{--<option value="رياضه" {{($event->event_type == 'مباريات')?'selected':''}}>مباريات</option>--}}
                    {{--<option value="لا للفراغ" {{($event->event_type == 'لا للفراغ')?'selected':''}}>لا للفراغ</option>--}}
                    {{--<option value="عام" {{($event->event_type == 'عام')?'selected':''}}>عام</option>--}}
                    {{--<option value="فريق" {{($event->event_type == 'فريق')?'selected':''}}>فريق</option>--}}
                    {{--<option value="جروب" {{($event->event_type == 'جروب')?'selected':''}}>جروب</option>--}}
                {{--</select>--}}
            {{--</div>--}}
        {{--</div>--}}
            ...
            <div class="col-md-10 ls-group-input">
                <select id="drop" class="form-control" name="team_id">
                    ...
                    @foreach($teams as $team)
                        <option value="{{$team->id}}" {{($event->team_id == $team->id)?'selected':''}}>{{$team->name}} </option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-md-10">
                <input type="text" name="num_of_attendees" class="form-control active"
                       value="{{$event->num_of_attendees}}">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-md-10 ls-group-input">
                <div>
                        ...
                        <div class="file-preview">
                            ...
                            <div class="file-preview-thumbnails">
                                <div class="file-preview-frame"><img
                                            src="{{asset('/'.$event->image)}}" name="eventImg"
                                            class="file-preview-image" title="{{$event->image}}"
                                            alt="{{$event->image}}"></div>
                            </div>
                    ...
                    <input class="btn btn-ls btn-file" id="file-3" name="eventImg" type="file" value=""
