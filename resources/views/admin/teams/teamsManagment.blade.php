        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h1 class="panel-title">قائمة الفرق  لهذا الموقع</h1>
                </div>
                    ...
                    <div class="ls-editable-table table-responsive ls-table">
                        <table class="table table-bordered table-striped" id="ls-editable-table">
                            <div style="text-align:center;">
                                <a href="{{url('admin/teams/create ')}}" class="btn btn-default"
                                   style="border-radius:10px; font-size: 16px ">
                                    <i style="color: #8B8986" class="fa fa-group"></i> اضافة فريق جديد </a>
                            </div>
                            <thead>
                            <tr class="active">
                                <th class="text-center">#</th>
                                <th class="text-center">اسم الفريق</th>
                                <th class="text-center">وصف الفريق</th>
                                <th class="text-center">الحالة</th>
                                <th class="text-center">شعار الفريق</th>
                                <th class="text-center">ادمن الفريق</th>
                            ...
                            @foreach($teams as $team)
                                <tr>
                                    <td style="vertical-align: middle" class="text-center">{{$team->id}}</td>
                                    <td style="vertical-align: middle" class="text-center">{{$team->name}}</td>
                                    <td style="vertical-align: middle;max-width: 487px" class="text-center">{{$team->description}}</td>
                                    <td style="vertical-align: middle" class="text-center">{{($team->status == 0)?'غير مفعل': 'مفعل'}}</td>

                                    @if($team->slogan != NULL)
                                        <td style="width:60px ;"><img style="display:block;width:100% ; height:40px"
                                                                      src="{{asset('/'.$team->slogan)}}"
                                                                      alt="{{$team->slogan}}"></td>
                                    @else
                                        <td style="width:60px ;"><img style="display:block;width:100% ; height:40px"
                                    ...
                                    <td style="vertical-align: middle" class="text-center">{{$team->teamAdmin->name}}</td>
                                    <td style="vertical-align: middle" class="text-center"><a href="{{url('admin/teams/show/media/'.$team->id)}}"> ميديا </a></td>
                                    <td style="vertical-align: middle;max-width: 91px;" class="text-center">

                                        <form style="margin-bottom: 3px;" method="post" action="{{url('admin/teams/'.$team->id)}}">
                                            {!! csrf_field() !!}
                                            {!! method_field('DELETE') !!}
                                            <button onclick="return confirm('هل انت متأكد من حذف بيانات هذا الفريق ؟ ')" style="border-radius:5px;" class="btn btn-xs btn-danger"><i
                                                        class="fa fa-trash-o"></i>
                                                حذف
                                            </button>
                                        </form>

                                        <a href="{{url('admin/teams/'.$team->id.'/edit')}}" style="border-radius:5px;margin-bottom: 3px;"
                                           class="btn btn-xs btn-warning"><i
                                                    class="fa fa-pencil-square-o"> تعديل </i></a>
                                        @if($team->status == 0)
                                            <a href="#" style="border-radius:5px;"
                                               class="btn btn-xs btn-success" onclick="unblockRedirect({{$team->id}})"><i
                                                        class="fa fa-check"> تفعيل </i></a>
                                        @elseif($team->status == 1)
                                            <a href="#" style="border-radius:5px;"
                                               class="btn btn-xs btn-danger" onclick="blockRedirect({{$team->id}})"><i
