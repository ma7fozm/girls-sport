        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h1 class="panel-title">جميع التعليقات الخاصه بمقال<bold>{{$article->title}}</bold> </h1>
                </div>
                <div class="panel-body">
                    ...
                    <div class="ls-editable-table table-responsive ls-table">
                        <table class="table table-bordered table-striped" id="ls-editable-table">
                            ...
                            @foreach($comments as $comment)
                                <tr>
                                    ...
                                    <td style="vertical-align: middle"
                                        class="text-center">{{($comment->status == 0)?'محظور': 'مفعل'}}</td>
                                    <td style="vertical-align: middle" class="text-center"><a href="{{url('admin/articles/comments/replies/'.$comment->id)}}"> الردود </a></td>
                                    <td style="vertical-align: middle" class="text-center">
                                        <form method="post" action="{{url('admin/articles/comment/'.$comment->id)}}">
                                            {!! csrf_field() !!}
                                            {!! method_field('DELETE') !!}
                                        ...
                                        @if($comment->status == 0)
                                            <a onclick="return confirm('هل تريد تفعيل هذا التعليق  ؟ ')"
                                               href="{{url('admin/articles/comment/unblock/'.$comment->id)}}" style="border-radius:5px;" class="btn btn-xs btn-success" >
                                                <i class="fa fa-check"> تفعيل </i></a>
                                        @elseif($comment->status == 1)
                                            <a onclick="return confirm('هل انت متأكد من حظر هذا التعليق ؟ ')"
                                               href="{{url('admin/articles/comment/block/'.$comment->id)}}" style="border-radius:5px;" class="btn btn-xs btn-danger" >
                        ...
                        {{url('admin/articles/comment/addcomment/'.$article->id)}}"
                              class="ls_form"
                              style="width: 700px ; margin: 0 auto; ">
                            {!! csrf_field() !!}
                            <div class="form-group">
                                <label style="vertical-align: middle" class="col-md-2 control-label">اكتب  تعليقك </label>

                                <div class="col-md-12">
                                <textarea name="reply" style="margin-right: 166px"
                                          class="form-control active" required>{{old('reply')}}</textarea>
                                </div>
                            </div>
