        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h1 class="panel-title"> الميديا الخاصة  بمجموعة <bold>{{$groupName}}</bold> </h1>
                    ...
                    <div class="ls-editable-table table-responsive ls-table">
                        <table class="table table-bordered table-striped" id="ls-editable-table">
                            <div style="text-align:center;">
                                <a href="{{url('admin/groups/media/add/'.$group_id)}}" class="btn btn-default"
                                   style="border-radius:10px; font-size: 16px ">
                                    <i style="color: #8B8986" class="fa fa-image"></i> اضافة صورة جديدة </a>
                                    <a href="{{url('admin/groupsv/media/add/'.$group_id)}}" class="btn btn-default"
                                   style="border-radius:10px; font-size: 16px ">
                                ...
                                <tr>
                                    ...
                                    <td style="width: 235px;" class="text-center">
                                        ...
                                        <form method="post" action="{{url('admin/media/'.$media->id)}}">
                                            {!! csrf_field() !!}
                                            {!! method_field('DELETE') !!}
                                            <button onclick="return confirm('هل انت متأكد من حذف  تلك الميديا نهائيا لتلك المجموعة ؟ ')" style="border-radius:5px;" class="btn btn-xs btn-danger"><i
...
@section('js')
    ...
    <script>
        ...
        function blockFunction() {
            return confirm("هل انت متأكد من حظر الميديا لهذه المجموعة ؟");
        }

        function unblockFunction() {
            return confirm("هل تريد تفعيل تفعيل الميديا لهذه المجموعة ؟");
        }
