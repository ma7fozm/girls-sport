...
@section('content')
    ...
    <form id="ffrm" role="form" enctype="multipart/form-data" method="post" action="{{url('admin/event/album/'.$album->id)}}"
          class="ls_form"
            ...
            <div class="col-md-10 ls-group-input">
                ...
                <select class="form-control" name="event_id">
                    ...
                    @foreach($events as $event)
                        <option value="{{$event->id}}" {{($album->event->id == $event->id)?'selected':''}} >{{$event->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-md-10">
                <input type="text" name="name" class="form-control active" value="{{$album->name}}">
            </div>
        </div>
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12">
                        <div class="radio-button-place">
                            <label style="margin-right: 10px; margin-bottom: 10px"
                                ...
                                <input type="radio" name="status" value="1" {{($album->status == '1')?'checked':''}}>
                                مفعل
                            </label>
                            <label style="margin-right: 35px;margin-bottom: 10px;"
                                   class="radio-inline icheck-radio-inline">
                                <input type="radio" name="status" value="0" {{($album->status == '0')?'checked':''}}>
                                غير مفعل
                            </label>
