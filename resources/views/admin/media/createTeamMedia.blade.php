...
@section('content')
    ...
    <h1 class="text-center"> <i class="fa fa-image"> اضافة ميديا الى الفريق </i></h1>
    <form id="ffrm" role="form" enctype="multipart/form-data" method="post" action="{{url('admin/teams/media')}}" class="ls_form"
                ...
                @if(isset($team_id))
                    <select class="form-control" name="team_id">
                        ...
                        @foreach($teams as $team)
                            <option value="{{$team->id}}" {{($team->id == $team_id)?'selected':''}}>{{$team->name}}</option>
                        @endforeach
                    </select>
                @else
                    <select class="form-control" name="team_id">
