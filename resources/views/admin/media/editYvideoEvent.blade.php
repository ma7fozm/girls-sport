...
@section('content')
    ...
    <h1 class="text-center"><i class="fa fa-edit"> تعديل بيانات الفيديو </i></h1>
    <form role="form" enctype="multipart/form-data" method="post" action="{{url('admin/eventsVid/media/update/'.$media->id)}}"
          class="ls_form"
        ...
        <div class="form-group">
            <label class="col-md-2 control-label">الفاعلية</label>
            <div class="col-md-10">
                <input type="text" readonly name="event_id" class="form-control active" value="{{$event->name}}">
            </div>
        </div>
            ...
            <div class="col-md-10 ls-group-input">
                <select id="albumSelect" class="form-control" name="album_id">
                    @foreach($albums as $album)
                        <option value="{{$album->id}}"{{($album->id == $media->album[0]->id)?'selected':''}}>{{$album->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
