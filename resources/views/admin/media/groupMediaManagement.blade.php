        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h1 class="panel-title">الميديا الخاصة بالمجموعات</h1>
                </div>
                    ...
                    <div class="ls-editable-table table-responsive ls-table">
                        <table class="table table-bordered table-striped" id="ls-editable-table">
                            <div style="text-align:center;">
                                <a href="{{url('admin/groups/media/create')}}" class="btn btn-default"
                                   style="border-radius:10px; font-size: 16px ">
                                    <i style="color: #8B8986" class="fa fa-image"></i> اضافة صورة جديدة </a>
                                     <a href="{{url('admin/groupsVid/media/create')}}" class="btn btn-default"
                                   style="border-radius:10px; font-size: 16px ">
                                    <i style="color: #8B8986" class="fa fa-youtube-play"></i> إضافه فيديو جديد من موقع اليوتيوب </a>
                            </div>
                            <thead>
                            <tr class="active">
                            ...
                            @foreach($medias as $media)
                                <tr>
                                    ...
                                    <td class="text-center">{{$media->group->name}}</td>
                                    <td class="text-center">{{($media->status == 0)?'غير مفعل':'مفعل'}}</td>
                                    <td class="text-center">{{$media->added_user->name}}</td>
                                      <td style="vertical-align: middle" class="text-center"><a href="{{url('admin/media/show/comments/'.$media->id)}}"> تعليقات </a></td>
                                    ...
                                    <td style="width:160px;" class="text-center">
                                        ...
                                        <form method="post" action="{{url('admin/media/'.$media->id)}}">
                                            {!! csrf_field() !!}
                                            {!! method_field('DELETE') !!}
                                            <button onclick="return confirm('هل انت متأكد من حذف  تلك الميديا نهائيا من الموقع لتلك المجموعة ؟ ')" style="border-radius:5px;" class="btn btn-xs btn-danger"><i
                                                        class="fa fa-trash-o" ></i>
                                                حذف
                                            </button>
                                        </form>
                                  @if($media->type == 'فيديو')
                                       <a href="{{url('admin/groupsVid/media/'.$media->id.'/edit')}}" style="border-radius:5px;"
                                           class="btn btn-xs btn-warning"><i
                                                    class="fa fa-pencil-square-o"> تعديل </i></a>
                                                    @else
                                       <a href="{{url('admin/groups/media/'.$media->id.'/edit')}}" style="border-radius:5px;"
                                           class="btn btn-xs btn-warning"><i
                                                    class="fa fa-pencil-square-o"> تعديل </i></a>
                                                    @endif
...
@section('js')
    ...
    <script>
        ...
        function blockFunction() {
            return confirm("هل انت متأكد من عدم تفعيل ظهور تلك الميديا بالمجموعة ؟");
        }

        function unblockFunction() {
            return confirm("هل تريد تفعيل الميديا لظهورها بالمجموعة ؟");
        }
