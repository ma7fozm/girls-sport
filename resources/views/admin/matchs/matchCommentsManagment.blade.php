        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h1 class="panel-title"> جميع التعليقات الخاصة بمباراة <bold>{{$match->title}}</bold> </h1>
                </div>
                        ...
                        <table class="table table-bordered table-striped" id="ls-editable-table">
                            ...
                            @foreach($comments as $comment)
                                <tr>
                                    ...
                                    <td style="vertical-align: middle"
                                        class="text-center">{{($match->status == 0)?'محظور': 'مفعل'}}</td>
                                    <td style="vertical-align: middle" class="text-center"><a href="{{url('admin/match/comments/replies/'.$comment->id)}}"> الردود </a></td>
                                    <td style="vertical-align: middle" class="text-center">
                                        <form method="post" action="{{url('admin/matchs/comment/'.$comment->id)}}">
                                            {!! csrf_field() !!}
                                            {!! method_field('DELETE') !!}
                                        ...
                                        @if($comment->status == 0)
                                            <a onclick="return confirm('هل تريد تفعيل هذا التعليق  ؟ ')"
                                               href="{{url('admin/matchs/comment/unblock/'.$comment->id)}}" style="border-radius:5px;" class="btn btn-xs btn-success" >
                                                <i class="fa fa-check"> تفعيل </i></a>
                                        @elseif($comment->status == 1)
                                            <a onclick="return confirm('هل انت متأكد من حظر هذا التعليق ؟ ')"
                                               href="{{url('admin/matchs/comment/block/'.$comment->id)}}" style="border-radius:5px;" class="btn btn-xs btn-danger" >
