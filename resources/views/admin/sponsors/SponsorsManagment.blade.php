        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h1 class="panel-title">قائمه الرعاه لهذا الموقع</h1>
                </div>
                    ...
                    <div class="ls-editable-table table-responsive ls-table">
                        <table class="table table-bordered table-striped" id="ls-editable-table">
                            <div style="text-align:center;">
                                <a href="{{url('admin/Sponsors/create')}}" class="btn btn-default"
                                   style="border-radius:10px; font-size: 16px ">
                                    <i style="color: #8B8986" class="fa fa-user"></i>اضافه راعي جديد</a>
                            </div>
                            <thead>
                            <tr class="active">
                                <th class="text-center">#</th>
                                <th class="text-center">اسم الراعي</th>
                                <th class="text-center">صوره الراعي</th>
                            ...
                            @foreach($sponsors as $sponsor)
                                <tr>
                                    <td style="vertical-align: middle" class="text-center">{{$sponsor->id}}</td>
                                    <td style="vertical-align: middle" class="text-center">{{$sponsor->name}}</td>
                                       <td style="width:90px ;" ><img style="display:block;width:100% ; height:60px"
                                                                   src="{{asset($sponsor->image)}}"
                                                                   alt="{{$sponsor->image}}"></td>
                                    <td style="vertical-align: middle" class="text-center">{{($sponsor->status == 0)?'غير مفعل': 'مفعل'}}</td>

                                
                                    <td style="vertical-align: middle;max-width: 91px;" class="text-center">

                                        <form style="margin-bottom: 3px;" method="post" action="{{url('admin/Sponsors/'.$sponsor->id)}}">
                                            {!! csrf_field() !!}
                                            {!! method_field('DELETE') !!}
                                            <button onclick="return confirm('هل انت متأكد من حذف بيانات هذا الراعي؟ ')" style="border-radius:5px;" class="btn btn-xs btn-danger"><i
                                                        class="fa fa-trash-o"></i>
                                                حذف
                                            </button>
                                        </form>

                                        <a href="{{url('admin/Sponsors/'.$sponsor->id.'/edit')}}" style="border-radius:5px;margin-bottom: 3px;"
                                           class="btn btn-xs btn-warning"><i
                                                    class="fa fa-pencil-square-o"> تعديل </i></a>
                                        @if($sponsor->status == 0)
                                            <a href="#" style="border-radius:5px;"
                                               class="btn btn-xs btn-success" onclick="unblockRedirect({{$sponsor->id}})"><i
                                                        class="fa fa-check"> تفعيل </i></a>
                                        @elseif($sponsor->status == 1)
                                            <a href="#" style="border-radius:5px;"
                                               class="btn btn-xs btn-danger" onclick="blockRedirect({{$sponsor->id}})"><i
...
@section('js')
    ...
    <script>
        ...
        function unblockRedirect(user_id) {
            if (unblockFunction()) {
                path = "Sponsors/unblock/"
                window.location.href = path.concat(user_id);
            }
        }

        function blockFunction() {
            return confirm("هل انت متأكد من إلغاء تفعيل الراعي");
        }

        function unblockFunction() {
            return confirm("هل تريد تفعيل الراعي");
        }
