        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h1 class="panel-title">قائمة المجموعات  لهذا الموقع</h1>
                </div>
                    ...
                    <div class="ls-editable-table table-responsive ls-table">
                        <table class="table table-bordered table-striped" id="ls-editable-table">
                            <div style="text-align:center;">
                                <a href="{{url('admin/groups/create ')}}" class="btn btn-default"
                                   style="border-radius:10px; font-size: 16px ">
                                    <i style="color: #8B8986" class="fa fa-group"></i> اضافة مجموعة جديدة </a>
                            </div>
                            <thead>
                            <tr class="active">
                                ...
                                <th class="text-center">الرياضة</th>
                            ...
                            @foreach($groups as $group)
                                <tr>
                                    ...
                                    <td style="vertical-align: middle;" class="text-center">{{$group->name}}</td>
                                    <td style="vertical-align: middle;max-width: 487px" class="text-center">{{$group->description}}</td>
                                    ...
                                    <td style="vertical-align: middle;" class="text-center">{{($group->status == 0)?'غير مفعل': 'مفعل'}}</td>
                                    <td style="vertical-align: middle;" class="text-center">{{$group->groupAdmin->name}}</td>
                                    <td style="vertical-align: middle;" class="text-center">{{$group->sport->name}}</td>
                                    <td style="vertical-align: middle;" class="text-center">{{($group->team_id != NULL)?$group->team->name:'لا يوجد'}}</td>
                                    <td style="vertical-align: middle" class="text-center"><a href="{{url('admin/groups/show/media/'.$group->id)}}"> ميديا </a></td>

                                    <td style="vertical-align: middle;" style="width: 136px;" class="text-center">

                                        <form method="post" action="{{url('admin/groups/'.$group->id)}}">
                                            {!! csrf_field() !!}
                                            {!! method_field('DELETE') !!}
                                            <button onclick="return confirm('هل انت متأكد من حذف بيانات تلك المجموعة ؟ ')" style="border-radius:5px;margin-bottom: 5px;" class="btn btn-xs btn-danger"><i
                                                        class="fa fa-trash-o"></i>
                                                حذف
                                            </button>
                                        </form>

                                        <a href="{{url('admin/groups/'.$group->id.'/edit')}}" style="margin-bottom: 5px;border-radius:5px;"
                                           class="btn btn-xs btn-warning"><i
                                                    class="fa fa-pencil-square-o"> تعديل </i></a>
                                        @if($group->status == 0)
                                            <a href="#" style="border-radius:5px;margin-bottom: 5px;"
                                               class="btn btn-xs btn-success" onclick="unblockRedirect({{$group->id}})"><i
                                                        class="fa fa-check"> تفعيل </i></a>
                                        @elseif($group->status == 1)
                                            <a href="#" style="border-radius:5px; margin-bottom: 5px;"
                                               class="btn btn-xs btn-danger" onclick="blockRedirect({{$group->id}})"><i
...
@section('js')
    ...
    <script>
        ...
        function blockFunction() {
            return confirm("هل انت متأكد من الغاء تفعيل هذة المجموعة ؟");
        }

        function unblockFunction() {
            return confirm("هل تريد تفعيل هذة المجموعة ؟");
        }
