...
@section('content')
    ...
    <h1 class="text-center"><i class="fa fa-edit"> تعديل بيانات المجموعة </i></h1>
    <form role="form" enctype="multipart/form-data" method="post" action="{{url('admin/groups/'.$group->id)}}"
          class="ls_form"
        ...
        <div class="form-group">
            ...
            <div class="col-md-10">
                <input type="text" name="name" class="form-control active" value="{{$group->name}}">
            </div>
        </div>
        <div class="form-group">
            <label class="col-md-2 control-label">الوصف</label>
            <div class="col-md-10">
                <textarea name="description" class="form-control active">{{$group->description}}</textarea>
            </div>
        </div>
                        ...
                        <div class="radio-button-place">
                            <label style="margin-right: 10px;" class="radio-inline icheck-radio-inline">
                                <input type="radio" name="group_to"
                                       value="team" {{($group->team_id != NULL)?'checked':''}}>
                                فريق
                            </label>
                            <label style="margin-right: 35px;" class="radio-inline icheck-radio-inline">
                                <input type="radio" name="group_to"
                                       value="sport" {{($group->team_id == NULL)?'checked':''}}>
                                رياضة
                            </label>
                ...
                <div class="col-sm-10 flo-right ">
                    <select style="margin-bottom: 10px;margin-top: 7px;" name="sport_id" class="form-control">
                        ...
                        @foreach($sports as $sport)
                            <option value="{{$sport->id}}" {{($group->sport_id == $sport->id) ? 'selected':''}} >{{$sport->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="col-sm-10 flo-right ">
                    <select style="margin-bottom: 10px;" name="admin_id" id="drop" class="form-control">
                        ...
                        @foreach($users as $user)
                            <option value="{{$user->id}}" {{($group->team_id != NULL)?'':($group->admin_id == $user->id) ? 'selected':''}}>{{$user->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div id="team_div">
            <div class="form-group">
                ...
                <div class="col-sm-10 flo-right ">
                    <select style="margin-bottom: 10px;" name="team_id" id="drop" class="form-control">
                        <option value="">اختر...</option>
                        @foreach($teams as $team)
                            <option value="{{$team->id}}" {{$group->team_id == $team->id ? 'selected':''}} >{{$team->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="form-group">
            ...
            <div class="col-md-10 ls-group-input">
                            ...
                            <div class="file-preview-thumbnails">
                                <div class="file-preview-frame">
                                    @if($group->image_url != NULL)
                                        <img src="{{asset('/'.$group->image_url)}}" name="groupImg"
                ...
                <input class="btn btn-ls btn-file" id="file-3" name="groupImg" type="file" value=""
