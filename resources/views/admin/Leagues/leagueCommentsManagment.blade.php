...
@section('content')
        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h1 class="panel-title">جميع التعليقات الخاصه ب   <bold>{{$league->name}}</bold> </h1>
                </div>
                            ...
                            @foreach($comments as $comment)
                                <tr>
                                    ...
                                    <td style="vertical-align: middle"
                                     ...
                                     <td style="vertical-align: middle"
                                        class="text-center">{{($comment->status == 0)?'غير مفعل': 'مفعل'}}</td>
                                    <td style="vertical-align: middle" class="text-center"><a href="{{url('admin/league/comments/replies/'.$comment->id)}}"> الردود </a></td>
                                    <td style="vertical-align: middle" class="text-center">
                                        <form method="post" action="{{url('admin/league/comment/'.$comment->id)}}">
                                            {!! csrf_field() !!}
                                            {!! method_field('DELETE') !!}
                                        ...
                                        @if($comment->status == 0)
                                            <a onclick="return confirm('هل تريد تفعيل هذا التعليق  ؟ ')"
                                               href="{{url('admin/league/comment/unblock/'.$comment->id)}}" style="border-radius:5px;" class="btn btn-xs btn-success" >
                                                <i class="fa fa-check"> تفعيل </i></a>
                                        @elseif($comment->status == 1)
                                            <a onclick="return confirm('هل انت متأكد من حظر هذا التعليق ؟ ')"
                                               href="{{url('admin/league/comment/block/'.$comment->id)}}" style="border-radius:5px;" class="btn btn-xs btn-danger" >
 ...
 @if($league)
                        ...
                        {{url('admin/league/comment/addcomment/'.$league->id)}}"
                              class="ls_form"
