        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h1 class="panel-title">قائمة الاماكن  لهذا الموقع</h1>
                </div>
                    ...
                    <div class="ls-editable-table table-responsive ls-table">
                        <table class="table table-bordered table-striped" id="ls-editable-table">
                            <div style="text-align:center;">
                                <a href="{{url('admin/places/create ')}}" class="btn btn-default"
                                   style="border-radius:10px; font-size: 16px ">
                                    <i style="color: #8B8986" class="fa fa-placespaper-o"></i> اضافة مكان جديد </a>
                            </div>
                            <thead>
                            <tr class="active">
                                <th class="text-center">#</th>
                                <th class="text-center"> اسم المكان</th>
                                <th class="text-center">عنوان المكان</th>
                                <th class="text-center">صورة المكان</th>
                            ...
                            @foreach($places as $place)
                                <tr>
                                    ...
                                    <td style="width:90px ;" ><img style="display:block;width:100% ; height:60px"
                                                                   ...
                                                                   alt="{{$place->image}}"></td>
                                    <td class="text-center">{{($place->status == 0)?'غير مفعل': 'مفعل'}}</td>
                                    <td style="width: 175px;" class="text-center">

                                        <form method="post" action="{{url('admin/places/'.$place->id)}}">
                                            {!! csrf_field() !!}
                                            {!! method_field('DELETE') !!}
                                            <button onclick="return confirm('هل انت متأكد من حذف بيانات هذا المكان ؟ ')" style="border-radius:5px;" class="btn btn-xs btn-danger"><i
                                                        class="fa fa-trash-o"></i>
                                                حذف
                                            </button>
                                        </form>

                                        <a href="{{url('admin/places/'.$place->id.'/edit')}}" style="border-radius:5px;"
                                           class="btn btn-xs btn-warning"><i
                                                    class="fa fa-pencil-square-o"> تعديل </i></a>
                                        @if($place->status == 0)
                                            <a href="#" style="border-radius:5px;"
                                               class="btn btn-xs btn-success" onclick="unblockRedirect({{$place->id}})"><i
                                                        class="fa fa-check"> تفعيل </i></a>
                                        @elseif($place->status == 1)
                                            <a href="#" style="border-radius:5px;"
                                               class="btn btn-xs btn-danger" onclick="blockRedirect({{$place->id}})"><i
...
@section('js')
    ...
    <script>
        ...
        function blockFunction() {
            return confirm("هل انت متأكد من الغاء تفعيل هذا المكان ؟");
        }

        function unblockFunction() {
            return confirm("هل تريد تفعيل هذا المكان ؟");
        }
