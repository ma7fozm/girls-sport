            ...
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 flo-right dir">
                    <div class="col-lg-6 col-md-8 col-sm-8 col-xs-12 single-image flo-right">
                        <img class="new-hei" src="{{asset('/'.$new->image)}}" alt="">
                    </div>
                    ...
                    <div class="share-section">
                        <div class="row">
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 life-style flo-right">
                                <span class="date">
                                        <i class="fa fa-user-o" aria-hidden="true"></i> {{$new->user->name}}
                                    @if(count($new->comments)>0)
                                        <span class="date">  <i class="fa fa-comment-o" aria-hidden="true"></i> {{App\Http\Controllers\ArticleController::convertArabicNumbers(count($new->comments))}} </span>
                                    @endif
                                       </span>
                                <span class="date">
{{--                                <i class="fa fa-calendar-check-o" aria-hidden="true"></i> {{ \Arabicdatetime::date(strtotime($new->created_at) , 0 , 'j M Y'  ,'indian')}} </span>--}}
                                <i class="fa fa-calendar-check-o" aria-hidden="true"></i> {{ App\Http\Controllers\HomeController::ArabicDate(date('y-m-d', strtotime($new->created_at)))}} </span>
                            </div>
                    ...
                    <div class="author-comment">
                        ...
                        <ul>
                            @foreach($new->comments->where('parent','=',0) as $comment)
                                <li>
                                    <div class="row">
                                        ...
                                        <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 flo-right">
                                            ...
                                            @if(isset(Auth::user()->id))
                                                    @if($comment->added_by->id == Auth::user()->id)
                                                        ...
                                                        <form id="{{'my_form'.$comment->id}}"
                                                              ...
                                                              method="post"
                                                              action="{{url('/news/delete/comment/'.$new->id.'/'.$comment->id)}}">
                                            ...
                                            <div class="dsc-comments">
                                                ...
                                                <div class="add_comm">
                                                    <form method="post"
                                                          action="{{url('/news/add/reply/'.$new->id.'/'.$comment->id)}}">
                                                        {!! csrf_field() !!}
                                            ...
                                            @if(isset(Auth::user()->id))
                                                @if($reply->added_by->id == Auth::user()->id)
                                                    ...
                                                    <form id="{{'my_form'.$reply->id}}" style="display: inline-block;"
                                                          method="post"
                                                          action="{{url('/news/delete/comment/'.$new->id.'/'.$reply->id)}}">
                                                    {!! csrf_field() !!} {!! method_field('DELETE') !!}
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-10 col-xs-12 flo-right leave-comments-area">
                        ...
                        <form method="post"
                              action="{{url('/news/add/comment/'.$new->id)}}">
                            {!! csrf_field() !!}
                            <fieldset>
    ...
    <div class="modal fade" id="exampleModalc" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="edit_comment" method="post" action="{{url('/news/edit/comment/')}}">
                    {!! csrf_field() !!}
                    ...
                    <div class="modal-footer">
                        ...
                        <input type="hidden" id="customURL" value="{{url('news/'.$new->id.'/#scroll')}}">
                    </div>
                </form>
            </div>
        </div>
